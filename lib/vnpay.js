...
export const vnpayConfig = {
  ...
  vnp_ReturnUrl: process.env.VNP_RETURN_URL || "https://binhppshop.onrender.com/vnpay/return",
  vnp_IpnUrl: process.env.VNP_IPN_URL || "https://binhppshop.onrender.com/vnpay/ipn",
};
...
export function createPaymentUrl(params) {
  ...
  // Create sign data WITHOUT encoding (following VNPAY official documentation)
  // IMPORTANT: Do NOT encode when creating signature, only encode when building URL
  const signData = Object.keys(vnp_Params)
    .map(key => `${key}=${vnp_Params[key]}`)
    .join('&');
  console.log("🔐 Sign data (NOT encoded):", signData);
  ...
  // Build payment URL WITH encoding (following VNPAY official documentation)
...
export function verifyReturnUrl(vnpParams) {
  ...
  // Do NOT encode for signature verification (following VNPAY official documentation)
  ...
  console.log("🔍 Verifying return URL:");
...
export function verifyIpnCall(vnpParams) {
  ...
  console.log("  Calculated signature:", signed);
  console.log("  Received signature:", secureHash);
  console.log("  Match:", isValidSignature);
